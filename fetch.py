...
def run(market, mode):
    ...
    def quote(h, yf, mkt):
        """正确取"最新价"与"上一交易日收盘"作基准:
        - 历史最后一根是今天:基准用倒数第二根(昨收),最新价用盘中价或今天收;
        - 今天那根还没出但有盘中价:基准用最后一根(昨收),最新价用盘中价;
        - 休市:用最后一根 vs 前一根。"""
        c = h["c"]; dts = h.get("d") or []
        tz = US_TZ if mkt == "US" else CN_TZ
        today = datetime.now(tz).strftime("%m-%d")
        has_today = bool(dts) and dts[-1] == today
        ip = intr.get(yf)
        if has_today:
            return (ip if ip is not None else c[-1]), c[-2]
        if ip is not None:
            return ip, c[-1]
        return c[-1], c[-2]
    ...
    for r in records:
        ...
        data["history"][r["yf"]] = h
        last, prev = quote(h, r["yf"], r["market"])
        data["tickers"][r["yf"]] = {
